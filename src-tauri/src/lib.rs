...
#[command]
fn pairing_info(state: tauri::State<AppState>) -> Result<PairingInfo, String> {
    Ok(PairingInfo { device_id: state.device_id.clone(), device_name: state.device_name.clone(), ip: local_ip()?.to_string(), port: TRANSFER_PORT, token: state.token.clone() })
}
...
fn discover_devices(device_name: String, timeout_ms: u64) -> Result<Vec<NearbyDevice>, String> {
    ...
    let message = format!("{}|{}|{}", MAGIC, device_name, TRANSFER_PORT);
    socket.send_to(message.as_bytes(), (Ipv4Addr::BROADCAST, DISCOVERY_PORT)).map_err(|e| e.to_string())?;
    let deadline = Instant::now() + Duration::from_millis(timeout_ms.clamp(300, 3000));
    let mut found = Vec::new(); let mut buffer = [0u8; 1024];
    while Instant::now() < deadline { if let Ok((len, addr)) = socket.recv_from(&mut buffer) { let text = String::from_utf8_lossy(&buffer[..len]); let p: Vec<&str> = text.split('|').collect(); if p.len() == 3 && p[0] == MAGIC { if let Ok(port) = p[2].parse::<u16>() { found.push(NearbyDevice { name:p[1].to_string(), ip:addr.ip().to_string(), port }); } } } }
...
fn transfer_server(state: AppState) { let listener=match TcpListener::bind((Ipv4Addr::UNSPECIFIED,TRANSFER_PORT)){Ok(v)=>v,Err(_)=>return}; for incoming in listener.incoming(){ if let Ok(mut stream)=incoming { let s=state.clone(); thread::spawn(move||{ let (request,headers,mut body)=match read_headers(&mut stream){Ok(v)=>v,Err(_)=>return}; let token=header(&headers,"x-seam-token").unwrap_or_default(); if token!=s.token && !s.paired.lock().map(|p|p.contains(&token)).unwrap_or(false){ let _=stream.write_all(b"HTTP/1.1 401 Unauthorized\r\nContent-Length: 0\r\n\r\n"); return; }
 if request.starts_with("POST /pair "){ let _=stream.write_all(b"HTTP/1.1 200 OK\r\nContent-Length: 2\r\nContent-Type: application/json\r\n\r\n{}"); return; }
 if request.starts_with("POST /receive "){ let name=header(&headers,"x-file-name").unwrap_or_else(||"received-file".into()); let size=header(&headers,"content-length").and_then(|v|v.parse::<u64>().ok()).unwrap_or(0); let safe=name.chars().filter(|c|c.is_ascii_alphanumeric()||matches!(c,'.'|'_'|'-'|' ')).collect::<String>(); let safe=if safe.is_empty(){"received-file".into()}else{safe}; let mut path=PathBuf::from(std::env::var("USERPROFILE").unwrap_or_else(|_|".".into())); path.push("Downloads"); path.push("SEAM Share"); let _=fs::create_dir_all(&path); path.push(safe); if let Ok(mut file)=fs::File::create(path){ let mut total=body.len() as u64; let _=file.write_all(&body); let mut buf=[0u8;CHUNK]; while total<size { match stream.read(&mut buf){Ok(0)=>break,Ok(n)=>{let _=file.write_all(&buf[..n]);total+=n as u64;},Err(_)=>break} } let _=stream.write_all(b"HTTP/1.1 201 Created\r\nContent-Length: 2\r\n\r\nOK"); } } }); } } }
}

#[cfg_attr(mobile, tauri::mobile_entry_point)] pub fn run(){ let state=AppState{device_id:make_id(),device_name:"This PC".into(),token:make_id(),paired:Arc::new(Mutex::new(Vec::new()))}; let discovery_name=state.device_name.clone(); let transfer_state=state.clone(); thread::spawn(move||run_discovery_responder(discovery_name)); thread::spawn(move||transfer_server(transfer_state)); tauri::Builder::default().manage(state).invoke_handler(tauri::generate_handler![network_info,discover_devices,pairing_info]).run(tauri::generate_context!()).expect("error while running SEAM Share"); }
